...
class ServerConfig:
    ...
    name: str
    type: str  # "stdio" 或 "streamableHttp"
    ...
    def from_dict(cls, name: str, config: Dict[str, Any]) -> "ServerConfig":
        ...
        if server_type not in ("stdio", "streamableHttp"):
            raise ConfigValidationError(
                f"服务器 '{name}' 的类型 '{server_type}' 无效，"
                f"必须是 'stdio' 或 'streamableHttp'"
            )
    ...
    def is_http(self) -> bool:
        """判断是否为 HTTP 类型服务器"""
        return self.type == "streamableHttp"
...
class MCPConfig:
    ...
    def get_servers_by_type(self, server_type: str) -> List[ServerConfig]:
        ...
        Args:
            server_type: 服务器类型 ("stdio" 或 "streamableHttp")
...
def example_usage():
    ...
    print("\n>>> HTTP 服务器:")
    http_servers = config.get_servers_by_type("streamableHttp")
